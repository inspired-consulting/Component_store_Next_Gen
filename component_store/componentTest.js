...
const query = `INSERT INTO component
    ...
    VALUES ((SELECT COALESCE(MAX(id), 0) + 1 FROM component),'0e37df36-f698-11e6-8dd4-cb9ced3df976', 'test2', 'www.name.com') RETURNING id`
    var pool = pgpool.getPool();
    pool.query(query, (err, result) => {
    if (!err) { 
        console.log("insert value", result.rows[0].id); 
    }
    else {
        console.log(err.message); 
    }
