...
const { sortComponentByNewest, searchComponentByName, sortComponentAlphabetically } = require('../models/component');
...
router.get('/', (req, res) => {
    ...
    if (data.sort !== undefined && data.sort === 'abc') {
        ...
        sortComponentByNewest().then(rows => {
            console.log('input sortComponentByNewest',rows);
            const component = rows.length > 0 ? rows : false;
            res.render('showcomponents', {
                url: '/',
                component,
